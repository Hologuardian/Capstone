...
{
    //Credit to Giawa <https://www.giawa.com/magicavoxel-c-importer/>
    // this is the default palette of voxel colors (the RGBA chunk is only included if the palette is differet)
    ...
    /// <summary>
    /// Load a MagicaVoxel .vox format file into a Color32 array
    /// </summary>
    ...
    public static Color32[] FromMagica(BinaryReader stream)
    {
        // check out http://voxel.codeplex.com/wikipage?title=VOX%20Format&referringTitle=Home for the file format used below
        // we're going to return a voxel chunk worth of data
        Color32[] data = new Color32[32 * 128 * 32];
        Color32[] colors = null;
        ...
        if (magic == "VOX ")
        {
            ...
            for (int i = 0; i < voxelData.Length; i++)
            {
                ...
                if (voxelData[i].x > sizex || voxelData[i].y > sizey || voxelData[i].z > sizez) continue;

                // use the voxColors array by default, or overrideColor if it is available
                int voxel = (voxelData[i].x * sizez * sizey + voxelData[i].y * sizez + voxelData[i].z);
                data[voxel] = (colors == null ? ToColor(voxColors[voxelData[i].color - 1]) : colors[voxelData[i].color - 1]);
            }
        }
    ...
    //Pull apart the 16bit RGB into the unity RGBA
    private static Color32 ToColor(ushort color)
    {
        return new Color32((byte)(((float)((color >> 10) & 0x1f) / 31.0f) * 255.0f),
            (byte)(((float)((color >> 5) & 0x1f) / 31.0f) * 255.0f),
            (byte)(((float)(color & 0x1f) / 31.0f) * 255.0f), 
            (byte)255);
    }
}
